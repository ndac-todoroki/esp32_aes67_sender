...
void app_main(void)
{
  ...
  for (;;)
  {
    ...
    {
      printf("%x %x %x %x\n", DMA_READOUT[i * 4], DMA_READOUT[i * 4 + 1], DMA_READOUT[i * 4 + 2], DMA_READOUT[i * 4 + 3]);

      /* https://cdn-shop.adafruit.com/product-files/3421/i2S+Datasheet.PDF#page=7 */
      mic24_value = (((DMA_READOUT[i * 4 + 3] & 0xff) << 10) |
                     ((DMA_READOUT[i * 4 + 2] & 0xff) << 2) |
                     ((DMA_READOUT[i * 4 + 1] & 0xff) << 6)) &
                    0x00ffffff;

      printf("%x", mic24_value);
